...
namespace Aurora
{
	...
	void AllocBufferMemory(vk::Buffer buffer, vk::DeviceMemory* memory)
	{
		vk::MemoryRequirements mem_reqs;
	...
	Handle CreateVertexBufferHandle(const void* data, int32 size)
	{
		...
		if (res.result != vk::Result::eSuccess) {
			GLog->Error("vulkan create vertex buffer failed result :%d", res.result);
			return -1;
		}

		vk::DeviceMemory memory;
		AllocBufferMemory(res.value, &memory);

		if (data != nullptr) {
			void* pData = nullptr;
			auto result = device.mapMemory(memory, 0, VK_WHOLE_SIZE, vk::MemoryMapFlags(), &pData);
			VERIFY(result == vk::Result::eSuccess);
	...
	Handle CreateIndexBufferHandle(const void* data, int32 size)
	{
		...
		auto res = device.createBuffer(createInfo);
		if (res.result != vk::Result::eSuccess) {
			GLog->Error("vulkan create index buffer failed result :%d", res.result);
			return -1;
		}
